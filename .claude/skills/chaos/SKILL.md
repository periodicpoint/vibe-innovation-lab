...
1. **Invocation context.** This skill can be invoked standalone (the user typed `/chaos` outside any phase) or from Phase 2 (Step 2.5). The protocol document handles both cases in its handoff step. If the conversation is already inside a Phase 2 run, treat the invocation as Phase 2 Step 2.5. Otherwise treat it as standalone.
