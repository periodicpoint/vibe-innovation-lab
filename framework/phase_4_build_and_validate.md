...
2. If this is the team's first iteration and the app tab has not opened yet, say: "The Streamlit app starts automatically when the Codespace finishes loading. It runs as a background process spawned by the devcontainer's postStartCommand and opens in a new browser tab by itself. If you do not see it after about ten seconds, check the log file with `tail -f /tmp/streamlit.log` in any terminal. The forwarded URL is also printed at the top of every new terminal you open."
...
4. If an error occurs, say: "Copy the error message from the Streamlit app in the browser, or from `/tmp/streamlit.log` via `tail -n 50 /tmp/streamlit.log`, and paste it here. I will fix it."
5. If the Streamlit process died completely and hot reload no longer works, say: "Run `pkill -f 'streamlit run'` in a terminal to clear any stale process, then run `cd prototype && uv run streamlit run app.py`. If `uv` is not available (the devcontainer used the pip fallback on creation), run `streamlit run prototype/app.py` from the repository root instead. This starts Streamlit in the foreground of your terminal so you see the logs directly."
