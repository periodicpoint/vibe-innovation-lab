...
5. The Streamlit app starts automatically as soon as the Codespace finishes loading. The launch happens inside the `postStartCommand` in [.devcontainer/devcontainer.json](../.devcontainer/devcontainer.json), which spawns Streamlit as an orphaned background process during container start. The command auto-selects `uv run streamlit run app.py` when `uv` is available and falls back to plain `streamlit run app.py` when the pip path was used during container creation. No popup, no consent prompt, no manual click. Output is redirected to `/tmp/streamlit.log`, prefixed with a UTC timestamp marker at each start so stale and fresh log data can be told apart. If the app does not come up, inspect the log with `tail -f /tmp/streamlit.log` in the terminal. If you need to restart Streamlit manually, kill the running process with `pkill -f 'streamlit run'` and start a fresh one in a terminal with:
